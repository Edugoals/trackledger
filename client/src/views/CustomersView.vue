<template>
  <div class="view customers-view">
          ...
          <template v-if="selectedJob">
            ...
            <ul class="list">
              <li v-for="t in tracks" :key="t.id" class="list-item">
                <router-link :to="{ name: 'track', params: { projectId: selectedJob.id, trackId: t.id } }" class="track-link">
                  {{ t.name }}
                </router-link>
                <div class="item-actions">
                  <button type="button" class="btn-small" @click.stop="openTrackForm(t)">Bewerk</button>
                  <button type="button" class="btn-small danger" @click.stop="deleteTrack(t.id)">Verwijder</button>
                </div>
              </li>
            </ul>
    ...
    <div v-if="showCalendarForm" class="modal">
...
async function selectCustomer(c) {
  ...
  headerContext.selectedCustomerId = c.id
  selectedJob.value = null
...
function loadCalendars() {
  return api('/api/calendars').then(r => r.ok && r.json().then(arr => { calendars.value = arr }))
}
...
onMounted(() => {
  loadCustomers()
  window.addEventListener('trackledger-sync-done', onSyncDone)
})
onUnmounted(() => {
  window.removeEventListener('trackledger-sync-done', onSyncDone)
  headerContext.selectedCustomerId = null
})
function onSyncDone() {
  if (selectedCustomer.value?.id) loadEvents()
}
</script>

<style scoped>
.customers-view h2 { margin: 0 0 1rem; }
.customers-view h3 { margin: 1rem 0 0.5rem; }
...
.track-link { flex: 1; text-decoration: none; color: inherit; font: inherit; }
.track-link:hover { text-decoration: underline; }
.item-actions { display: flex; gap: 0.25rem; }
.btn-small { padding: 0.25rem 0.5rem; font-size: 0.85rem; background: #f3f4f6; border: 1px solid #d1d5db; border-radius: 4px; cursor: pointer; }
...
.modal { position: fixed; inset: 0; background: rgba(0,0,0,0.4); display: flex; align-items: center; justify-content: center; z-index: 10; }
